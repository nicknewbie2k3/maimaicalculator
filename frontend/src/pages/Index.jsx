...
function isNewChart(song, dict) {
  const v = song.version || (dict[song.song_name] || {}).version || ''
  return v === 'PRiSM PLUS' || v === 'CiRCLE'
}

function diffClass(d) {
  return { basic: 'diff-basic', advanced: 'diff-advanced', expert: 'diff-expert', master: 'diff-master', 're:master': 'diff-remas', remaster: 'diff-remas' }[d.toLowerCase()] || ''
}

function recategorize(data, dict) {
  const out = { old_songs: [], new_songs: [] }
  const add = (song, arr, max) => {
  ...
  ;[...data.old_songs, ...data.new_songs].forEach(s => {
    ...
    add(s, isNew ? out.new_songs : out.old_songs, isNew ? 15 : 35)
  })
    ...
    <td className={`song-grid-cell ${diffClass(song.difficulty_type)} ${chartClass}${divider ? ' border-divider-row' : ''}`}>
      <div className="song-grid-cell-inner">
        ...
        <div className="song-cell-bottom-left">
          ...
          <span className="song-achievement">{parseFloat(song.achievement).toFixed(4)}%</span><br />
          <span className="song-chart-diff-bold">{parseFloat(song.chart_difficulty).toFixed(1)}</span>
        </div>
...
function SongTable({ label, songs, idPrefix }) {
  return (
    <>
            ...
            {songs.length === 0
              ...
              : songs.map((s, i) => (
                <tr key={i} className={isNewChart(s, {}) ? 'new-chart-row' : 'old-chart-row'}>
                  ...
                  <td>{parseFloat(s.achievement).toFixed(4)}%</td>
                  <td>{parseFloat(s.chart_difficulty).toFixed(1)}</td>
...
export default function Index() {
  const { allSongNames, maimaiSongsDict, aliasToTitleMap } = usePage().props

  const [b50, setB50State] = useState({ old_songs: [], new_songs: [] })
  const [fullData, setFullDataState] = useState(null)
  ...
  const b50FileRef = useRef(null)
  const cacheFileRef = useRef(null)
  const gridRef = useRef(null)
  const statusTimerRef = useRef(null)

  const showStatus = useCallback((msg, type, ms = 4000) => {
    clearTimeout(statusTimerRef.current)
    setStatus({ msg, type, show: true })
    if (ms > 0) statusTimerRef.current = setTimeout(() => setStatus(s => ({ ...s, show: false })), ms)
  }, [])

  const readB50 = () => {
    try { return JSON.parse(localStorage.getItem(B50_KEY) || 'null') || { old_songs: [], new_songs: [] } } catch { return { old_songs: [], new_songs: [] } }
  }
  const writeB50 = (d) => { localStorage.setItem(B50_KEY, JSON.stringify(d)); setB50State(d) }
  const readFull = () => { try { return JSON.parse(localStorage.getItem(FULL_KEY) || 'null') } catch { return null } }
  const writeFull = (d) => { try { localStorage.setItem(FULL_KEY, JSON.stringify(d)); setFullDataState(d) } catch { } }
  ...
  async function handleB50File(e) {
  ...
  async function handleCacheFile(e) {
    const file = e.target.files[0]; if (!file) return
    ...
    } catch (err) { showStatus('Network error: ' + err.message, 'danger', 7000) }
    e.target.value = ''
  }
  ...
  async function saveToFile() {
    ...
    try {
      ...
      if (ct && ct.includes('application/json')) { const d = await res.json(); throw new Error(d.message) }
    ...
    } catch (err) { showStatus('Error: ' + err.message, 'danger', 5000) }
  }
  ...
  function downloadConverted() {
    const a = document.createElement('a'); a.href = '/download-converted-b50/'; a.download = ''
  ...
  const oldPad = [...display.old_songs]; while (oldPad.length < 35) oldPad.push(null)
  const newPad = [...display.new_songs]; while (newPad.length < 15) newPad.push(null)
              ...
              {Array.from({ length: 10 }, (_, row) => (
                <tr key={row}>
                  {Array.from({ length: 5 }, (_, col) => {
                    ...
                    return <SongCell key={col} song={song} songDict={maimaiSongsDict} divider={row === 7} />
                  })}
                </tr>
              ))}
            </tbody>
